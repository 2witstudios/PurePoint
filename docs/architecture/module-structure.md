...
**`pu-core` modules:** `agent_def`, `config`, `error`, `id`, `manifest`, `paths`, `protocol`, `schedule_def`, `serde_defaults`, `swarm_def`, `template`, `trigger_def`, `types`, `validation` — all public, no re-export flattening (callers use `pu_core::protocol::Request`, etc.). `protocol` and `types` are directories with submodules.
...
- Workspace-level dependency version management (`[workspace.dependencies]`: serde, serde_json, thiserror, chrono, tokio, uuid, indexmap)
