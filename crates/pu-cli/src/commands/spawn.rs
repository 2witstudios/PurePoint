...
pub async fn run(
    ...
    let (resolved_prompt, agent_override) = resolve_prompt(
        prompt,
        template_name,
        file,
        &var_map,
...
fn resolve_prompt(
    ...
    match (inline, template_name, file) {
        ...
        (None, Some(name), None) => {
            let tpl = template::find_template(project_root, &name).ok_or_else(|| {
                CliError::Other(format!("template not found: {name}"))
            })?;
        ...
        (None, None, Some(path)) => {
            let content = std::fs::read_to_string(&path).map_err(|e| {
                CliError::Other(format!("failed to read prompt file {path}: {e}"))
            })?;
            ...
            let agent_override = if tpl.agent != "claude" { Some(tpl.agent.clone()) } else { None };
            let rendered = template::render(&tpl, vars);
            Ok((rendered, agent_override))
        }
        (None, None, None) => {
            Err(CliError::Other("prompt required — provide inline, --template, or --file".into()))
        }
        _ => {
            Err(CliError::Other("provide only one of: inline prompt, --template, or --file".into()))
        }
    }
}
